...
def get_ui(request):
    ...
    return render_to_response('catalis-ui.html', {'settings': settings, 'lang': 'spa', 'lang2': 'es'})


@login_required
def get_config(request):
    '''Returns a JavaScript file with configuration settings.'''
    return render_to_response('config.js', {'request': request, 'settings': settings, 'lang': 'spa', 'lang2': 'es'})
...
def handle_db_access(request, db_name, **params):
    ...
    if params['action'] in ('get_record', 'search', 'do_list'):
        if not 'format' in params:
            params['format'] = 'full'
        response_data = _transform(result, params['format'])
    else:
...
def test_pages(request, page):
    return direct_to_template(request, template="test/%s.html" % page, extra_context={'settings': settings})
