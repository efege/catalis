...
class MarcDatabase(models.Model):
    ...
    def _init_isisdb(self):
        ...
        db_params = {
            ...
            'server': self.server,
            'fst': os.path.join(settings.PATH_DB, self.db_type + '.fst'),
            'actab': os.path.join(settings.PATH_DB, 'ac-ansi.tab'),
            'uctab': os.path.join(settings.PATH_DB, 'uc-ansi.tab')
        }
    ...
    def _get_dirname(self):
        ...
        '''
        return os.path.join(settings.PATH_DB, self.name)
    ...
    def _create_isis(self):
        ...
        try:
            ...
            os.mkdir(path)
            os.chmod(path, 0777)  # quick & dirty fix for the permissions problem -- FIXME
